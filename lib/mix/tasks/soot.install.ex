defmodule Mix.Tasks.Soot.Install.Docs do
  ...
  def long_doc do
    ...
    cd my_iot
    mix ash.setup
    mix soot.seed --demo   # optional fixture devices
    mix phx.server
    ```
    ...
    * `--example` / `--no-example` — when set (default: ON), each
      ...
      stream, weather shadow), and `mix soot.seed --demo` is scheduled
      to run after `ash.setup`. Pass `--no-example` for a clean
      skeleton (a default tenant + admin user are still seeded).
...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.Soot.Install do
    ...
    # Adds `:role` (default `:admin`) and `:tenant_id` to the User
    # resource generated by `ash_authentication.install`, an
    # internal `:register_admin` create action gated by a policy that
    # accepts `:seed` System actors or existing `:admin` users, and
    # flips the magic-link strategy's `registration_enabled?` to
    # `false`. Together these turn the User into an admin-only actor
    # whose tenant scope is read by the per-resource policies in
    # soot_core / ash_pki / soot_telemetry / soot_segments /
    # soot_contracts. Public sign-up is closed; admin provisioning
    # routes through `mix soot.seed` (System `:seed` actor) or an
    # existing admin onboarding a peer.
    defp patch_user_resource(igniter) do
      ...
      if exists? do
        ...
        attribute :role, :atom do
          ...
          allow_nil? false
          public? true
          constraints one_of: [:admin]
        end
        """)
        |> Ash.Resource.Igniter.add_new_attribute(user_module, :tenant_id, """
        attribute :tenant_id, :uuid do
          public? true
        end
        """)
        |> Ash.Resource.Igniter.add_new_action(user_module, :register_admin, """
        create :register_admin do
          description \"\"\"
          Magic-link admin provisioning. The user authenticates via
          the existing magic-link flow once their row exists; this
          action only creates the row. Restricted to the `:seed`
          System actor or an existing `:admin` user (see policy on
          this action) so disabling public registration on the
        ...
        |> Ash.Resource.Igniter.add_policy(
          user_module,
          quote(do: action(:register_admin)),
          quote do
            authorize_if(actor_attribute_equals(:part, :seed))
            authorize_if(actor_attribute_equals(:role, :admin))
          end
        )
        |> disable_registration_in_user(user_module)
      else
        igniter
      end
    end

    defp disable_registration_in_user(igniter, user_module) do
      path = Igniter.Project.Module.proper_location(igniter, user_module)
    ...
    # call in the operator's router. With registration disabled on
    # the magic-link strategy, the registration UI route would 404 on
    # POST anyway; clearing the option avoids the dead-link surface
    # in the sign-in template.
    defp patch_router_disable_registration(igniter) do
      {igniter, router} = Igniter.Libs.Phoenix.select_router(igniter)
      maybe_drop_register_path(igniter, router)
    end

    defp maybe_drop_register_path(igniter, nil), do: igniter

    defp maybe_drop_register_path(igniter, router) do
      path = Igniter.Project.Module.proper_location(igniter, router)

      if Igniter.exists?(igniter, path),
        do: Igniter.update_file(igniter, path, &drop_register_path/1),
        else: igniter
    end
    ...
    defp note_next_steps(igniter, options) do
      example_lines =
        if options[:example] do
          ...
          `mix soot.seed --demo` will run after `mix ash.setup` to
          create the default tenant, an admin user, and 5
          unprovisioned devices with pre-populated shadow desired
          state.

          Pass `--no-example` to skip the example resources and the
          demo fixtures (a default tenant + admin user are still
          seeded via `mix soot.seed`).
          """
        else
          ""
        end
      ...
      if options[:example] do
        Igniter.delay_task(igniter, "soot.seed", ["--demo"])
      else
        Igniter.delay_task(igniter, "soot.seed")
      end
    end
  end
else
