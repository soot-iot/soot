...
stage_seed() {
  ...
  # `mix ash.setup` composes `ash_postgres.create` (creates the DB)
  # and `ash_postgres.migrate` (runs migrations). The first migration
  # is the extensions migration generated from the consumer Repo's
  # `installed_extensions/0` callback — `["ash-functions", "citext"]`
  # after `soot.install` runs against a fresh project — so the
  # `CREATE EXTENSION IF NOT EXISTS "citext"` runs before any
  # users-with-citext migration. Previously the script pre-ran
  # `CREATE EXTENSION citext` via psql to work around a marker-skip
  # bug in `soot.install` that left the consumer Repo missing
  # `citext` in `installed_extensions/0`. Bug fixed in soot
  # `installer/citext-extension`; the workaround is no longer needed.
  log "mix ash.setup"
  mix ash.setup
